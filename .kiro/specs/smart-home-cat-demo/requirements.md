...
- **Cat_Profile_Service**: A Spring Boot (Java) microservice managing cat profiles, owner information, and cat health records
- **Device_Service**: A Go microservice managing IoT device registration, configuration, state persistence, and device commands via REST API. Maintains device shadow (desired/reported state) in DynamoDB and handles command execution by updating desired state.
- **Feeding_Service**: A Python Django microservice managing feeding schedules, portion control, and feeding history for cats
- **Health_Monitor_Service**: A Python Django microservice tracking cat health metrics from device telemetry data (weight, activity, litter box usage) stored in DynamoDB
...
- **EKS_Cluster**: Amazon Elastic Kubernetes Service cluster for deploying Spring Boot microservices
- **ECS_Fargate**: Amazon ECS with Fargate launch type for deploying containerized frontend applications and the Device Simulator
- **DynamoDB**: Amazon DynamoDB tables for storing device state, feeding schedules, and cat profiles
...
- **Sibling_Infra_Project**: An external Terraform project (in a sibling repository) that provisions the shared VPC, EKS_Cluster, ECS_Fargate, OIDC provider, and IAM roles; this application project assumes all infrastructure already exists
...
- **Deployment_Artifacts**: Dockerfiles, Kubernetes manifests, Helm charts, and ECS task definitions produced by this project for deployment to infrastructure provisioned by the Sibling_Infra_Project
...
1. THE Cat_Profile_Service SHALL store cat profiles containing name, breed, age, weight, dietary restrictions, and owner association
...
3. THE Health_Monitor_Service SHALL aggregate telemetry data from IoT devices (activity tracker, litter box monitor) into per-cat health summaries
4. WHEN a health metric for a cat deviates beyond a configurable threshold, THE Health_Monitor_Service SHALL generate a health alert and make the alert available to the AI agent implementations
5. THE Feeding_Service SHALL manage per-cat feeding schedules with configurable meal times, portion sizes, and dietary constraints
...
2. WHEN a feeding event completes, THE Feeding_Service SHALL record the event (timestamp, portion dispensed, device used, cat fed) in DynamoDB
...
4. WHEN a user requests feeding history via any AI agent implementation, THE Feeding_Service SHALL return the feeding log for the specified cat and time range
...
### Requirement 11: Polyglot Microservices on EKS

**User Story:** As a platform engineer, I want the backend microservices (built with Spring Boot, Python Django, and Go) deployed on EKS with proper service discovery and observability, so that the system demonstrates polyglot service development at scale.

#### Acceptance Criteria

1. THE API_Gateway_Service (Spring Boot), Cat_Profile_Service (Spring Boot), Device_Service (Go), Feeding_Service (Django), and Health_Monitor_Service (Django) SHALL each be deployed as separate Kubernetes deployments on the EKS_Cluster
2. THE EKS_Cluster SHALL use Kubernetes Service resources for internal service-to-service communication between microservices regardless of implementation language
3. WHEN a microservice instance becomes unhealthy, THE EKS_Cluster SHALL restart the instance using Kubernetes liveness and readiness probes
4. THE API_Gateway_Service SHALL route incoming requests to the appropriate downstream microservice based on the request path
5. WHEN a request passes through the API_Gateway_Service, THE API_Gateway_Service SHALL propagate trace context headers for distributed tracing
...
1. THE project SHALL include a Dockerfile for each microservice (Spring Boot, Django, and Go) that produces a container image deployable to the EKS_Cluster
2. THE project SHALL include Kubernetes manifests (or Helm charts) in a `deploy/k8s/` directory defining deployments, services, config maps, and ingress resources for all microservices on the EKS_Cluster
3. THE project SHALL include ECS task definitions and service configurations in a `deploy/ecs/` directory for deploying the Chatbot_UI, Admin_Console, and Device_Simulator as containerized applications on ECS_Fargate
4. THE project SHALL include a Dockerfile for each React frontend application that produces a container image serving the built static assets via a lightweight web server
5. THE project SHALL include deployment configuration files in a `deploy/` directory that reference environment-specific variables (e.g., VPC IDs, subnet IDs, cluster names) provided by the Sibling_Infra_Project
6. WHEN a developer builds any component, THE build system SHALL produce a deployment-ready artifact (container image) without requiring access to the infrastructure provisioning tools
...
1. THE project SHALL organize code into top-level directories: `langgraph-agent/` (LangGraph_Workflow with embedded RAG), `strands-agents/` (Strands_Agents multi-agent system), `chatbot/` (Chatbot_UI), `device-simulator/` (Device_Simulator), `admin-console/` (Admin_Console), `services/` (Spring Boot, Django, and Go microservices), `deploy/` (Kubernetes manifests, ECS task definitions), and `.github/workflows/` (CI_CD_Pipeline)
...
4. THE project SHALL include a local development setup using Docker Compose that starts the microservices, the Device_Simulator, the LangGraph_Workflow agent, and the Strands_Agents system for end-to-end testing without AWS dependencies
